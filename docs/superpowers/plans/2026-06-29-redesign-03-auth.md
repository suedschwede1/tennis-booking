# UI-Redesign Auth Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Login-Seite als Standalone-Seite mit Tailwind v4 umschreiben — kein Layout-Extend, Flux UI komplett ersetzt.

**Architecture:** `resources/views/auth/login.blade.php` wird komplett neu geschrieben als eigenständige HTML-Seite (kein `@extends`). Das Formular-Partial `_form.blade.php` wird nicht mehr includiert — der Formularinhalt wandert direkt in `login.blade.php`. `_form.blade.php` bleibt unverändert auf Disk.

**Tech Stack:** Laravel Blade, Tailwind CSS v4, Google Fonts (Red Hat Display + Text)
...
| Aktion | Datei |
|---|---|
| Modifizieren | `resources/views/auth/login.blade.php` |
| Unverändert | `resources/views/auth/_form.blade.php` |

---

### Task 1: `auth/login.blade.php` als Standalone-Seite neu schreiben
...
Lies `C:\development\bookingnew\resources\views/auth/login.blade.php` um den aktuellen Inhalt zu sehen.

Lies auch `C:\development\bookingnew\resources\views\auth\_form.blade.php` um zu verstehen welche Felder im Original vorhanden sind (insbesondere `name="alias"` für E-Mail und das `redirect_to` Hidden-Field).

- [ ] **Schritt 2: Datei komplett ersetzen**

Ersetze den gesamten Inhalt von `C:\development\bookingnew\resources\views\auth\login.blade.php` durch:
...
**Wichtig:** Das E-Mail-Feld verwendet `name="alias"` (nicht `name="email"`) — so erwartet es der `LoginController`.

- [ ] **Schritt 3: Commit**

```bash
git add resources/views/auth/login.blade.php
git commit -m "feat(auth): login.blade.php als Standalone-Seite mit Tailwind v4"
```
...
Erwartung: Kein Fehler. Die neue Tailwind-CSS enthält die neuen Klassen der Login-Seite (rounded-xl, grid-cols-2, accent-[#bf4316] etc.).

- [ ] **Schritt 2: Build committen**

```bash
git add public/build/
git commit -m "chore: Vite-Build nach Auth-Redesign aktualisiert"
```
...
Nach `npm run dev` im Browser `/login` öffnen:

1. Beige Hintergrund (`#f0ede6`), weiße Karte, zwei Spalten
2. Linke Spalte: „Mitgliedsbereich" Label, „Anmelden" Heading, Beschreibungstext
3. Rechte Spalte: E-Mail + Passwort Felder, Checkbox, Submit-Button (orange)
4. Submit mit falschen Daten → rote Fehler-Border an den Feldern
5. `redirect_to` URL-Parameter wird im Hidden-Field korrekt weitergegeben (testen mit `/login?redirect_to=/mein-konto`)
